...
void CheckMACD(){//MACD CONTROL
   ...
   if ((MACD_MainC > MACD_SignalC) && (MACD_MainP < MACD_SignalP)){
      CTB = true;
      Print("Buy");
   }
...
double MAsc2 = iMA(Symbol(), 60, 200, 0, MODE_EMA, PRICE_CLOSE, 0);
if (Open[0] > MAsc)
 {
 signal = "Buy";
 }
...
if (CTB && signal == "Buy" && OrdersTotal() == 0){
    ...
    OrderSend(_Symbol, OP_BUY, lotB, Ask, 0, Ask - int(100000*(Ask - Low[Lowest]) + 20)*_Point, Ask + (int(100000*(Ask - Low[Lowest]))*_Point * 150/100), NULL, 0, 0, Green);
}
  
if (CTS && signal == "Sell" && OrdersTotal() == 0){
    ...
    OrderSend(_Symbol, OP_SELL, -lotS, Bid, 0, Bid - int(100000*(Bid - High[Highest]) - 20)*_Point, Bid + (int(100000*(Bid - High[Highest]))*_Point * 150/100), NULL, 0, 0, Red);
}
